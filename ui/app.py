...
_APP_WISP_BACKGROUND_HTML = """
<iframe id="app-wisp-bg" src="/app/static/wisp/index.html" frameborder="0"></iframe>
"""
...
def _render_app_wisp_background() -> None:
    st.markdown(_APP_WISP_BACKGROUND_HTML, unsafe_allow_html=True)
...
def _render_board_area() -> None:
    ...
    if view == "play":
        ...
        if dropped_move:
            _handle_player_move(dropped_move, variant, depth)
        return
...
def _detect_explosions(old_fen: str, new_fen: str, move_uci: str) -> list[str] | None:
    ...
    # Atomic captures always remove at least the capturing and captured pieces.
    # Adjacent non-pawn pieces may also disappear, but a clean pawn capture can
    # still be exactly two disappeared squares.
